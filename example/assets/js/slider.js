const load_sliders = (settings) => {
        ...
        const animate = () => {
            if ((direction === 'right' &&
                ...
                (direction === 'left' &&
                    Math.abs(slider.translate_x) - Math.abs(settings.slider_speed) <= Math.abs(set_to_x))) {
                if (direction === 'right')
                    slider.translate_x -= Math.abs(set_to_x) - Math.abs(slider.translate_x);
                else
                    slider.translate_x += Math.abs(slider.translate_x) - Math.abs(set_to_x);
            }
            else {
                if (direction === 'right')
                    slider.translate_x -= settings.slider_speed;
                else
                    slider.translate_x += settings.slider_speed;
            }
            slider.slider_container_element.style.transform = `translateX(${slider.translate_x}px)`;
    ...
    const scroll_slider_child_to_nearest = (slider) => {
        ...
        const execute_when_available = (func) => {
    ...
    const prepare_slider = (slider, copy_count) => {
        let slider_elements_length = slider.slider_elements.length;
        if (slider_elements_length < 1)
            return;
        const get_slider_element_index = (index) => {
            if (index >= 0 && index < slider_elements_length)
                return index;
            if (index >= 0 && index >= slider_elements_length)
                return index - Math.floor(index / slider_elements_length) * slider_elements_length;
            return slider_elements_length - (Math.abs(index) - (Math.floor(Math.abs(index) / slider_elements_length)) * slider_elements_length);
        };
        const get_slider_element = (index) => {
    ...
    const slider_up_hook = (event, slider) => {
        const prepare_change_block = () => {
            if (slider.during_animation) {
                requestAnimationFrame(prepare_change_block);
                return;
            }
            change_slider_control_panel_block(slider, slider.slider_control_panel_blocks[slider.current_element_index - ELEMENTS_COPY_COUNT]);
        };
        scroll_slider_child_to_nearest(slider);
        slider.pressing = false;
        prepare_change_block();
    };
    const change_slider_control_panel_block = (slider, new_current_block) => {
    ...
    const load_slider_control_panel = (slider) => {
        const create_control_panel_block = (link_to_original_index) => {
        ...
        const block_click_hook = (e) => {
            if (slider.during_animation)
                return;
            const block_click_target = e.target;
        ...
        for (let i = 0; i < slider.child_elements_count; i++) {
            ...
            if (i === 0)
                control_panel_block.classList.add('current');
    ...
    slider_elements.forEach(slider_element => {
        ...
        const slider = {
            'slider_container_element': slider_container,
        ...
        listeners.forEach(listener_data => {
            listener_data.event_names.forEach(event_name => {
                slider_container.addEventListener(event_name, (e) => listener_data.function(e, slider));
            });
        });
